...
for iCh = 1:length(chTag)
    sig = smooth((inData.rawSignal(iCh, :) - nanmean(inData.rawSignal(iCh, bgInd), 2)) .* transpose(inData.height).^2, p.Results.gliding_window);
    sig(sig <= 0) = NaN;
...
for iCh = 1:length(chTag)
    sig = smooth((inData.rawSignal(iCh, :) - nanmean(inData.rawSignal(iCh, bgInd), 2)), p.Results.gliding_window);
