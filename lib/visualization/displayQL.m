...
%% log output
logFile = fullfile(config.resultPath, 'lidar_displayQL.log');
diaryon(logFile);
...
for iLidar = 1:length(lidarType)
    ...
    for iCh = 1:length(lidarConfig.chTag)
        ...
        lidarData.(['sig', lidarConfig.chTag{iCh}]) = h5read(h5Filename, ...
            sprintf('/sig%s', lidarConfig.chTag{iCh}));
    end
    ...
    case 11   % REAL
        ...
        % time slot
        tRange = [datenum(lidarConfig.tRange(1:19), 'yyyy-mm-dd HH:MM:SS'), ...
                  datenum(lidarConfig.tRange(23:41), 'yyyy-mm-dd HH:MM:SS')];
        if ~ isempty(lidarConfig.markTRange)
            tRangeMark = [datenum(lidarConfig.markTRange(1:19), 'yyyy-mm-dd HH:MM:SS'), ...
                          datenum(lidarConfig.markTRange(23:41), 'yyyy-mm-dd HH:MM:SS')];
        end
        ...
        vdr532 = lidarData.(['rcs', lidarConfig.vdrCompose{2}]) ./ ...
                 lidarData.(['rcs', lidarConfig.vdrCompose{1}]) * ...
                 lidarConfig.vdrCompose{3} + lidarConfig.vdrCompose{4};
        height = lidarData.height;
        mTime = lidarData.mTime(isChosen);
        deltaT = datenum(0, 1, 0, 0, lidarConfig.deltaT, 0);

        if ~ isempty(lidarConfig.markTRange)
            isChosenMark = (lidarData.mTime >= tRangeMark(1)) & ...
                           (lidarData.mTime <= tRangeMark(2));
            ...
            if ~ any(isChosenMark)
                ...
                vdr532Int = nanmean(lidarData.(['rcs', lidarConfig.vdrCompose{2}])(:, isChosenMark), 2) ./ ...
                            nanmean(lidarData.(['rcs', lidarConfig.vdrCompose{1}])(:, isChosenMark), 2) * ...
                            lidarConfig.vdrCompose{3} + lidarConfig.vdrCompose{4};
            end
        end
        ...
        if p.Results.flagCorTime
            ...
            for iT = 2:length(mTime)
                tInd = floor((mTime(iT) - mTime(iT - 1) + 0.1 * deltaT) / deltaT) + ...
                    tIndGrid(iT - 1);
        ...
        if ~ isempty(lidarConfig.markTRange)
            rectangle('Position', ...
                [tRangeMark(1), lidarConfig.hRange(1, 1), (tRangeMark(2) - tRangeMark(1)), (lidarConfig.hRange(1, 2) - lidarConfig.hRange(1, 1))], ...
                      'EdgeColor', 'k', ...
                      'LineWidth', 2, ...
                      'LineStyle', '--', ...
        ...
        set(gca, 'XMinorTick', 'on', ...
                 'YMinorTick', 'on', ...
                 'XTick', linspace(mTimeGrid(1), mTimeGrid(end), 5), ...
                 'Layer', 'Top', ...
                 'Box', 'on', ...
                 'TickDir', 'out', ...
                 'LineWidth', 2);
        ...
        if exist(config.resultPath, 'dir')
            export_fig(gcf, fullfile(config.resultPath, sprintf('quicklook_%s_%s.%s', lidarType{iLidar}, '532p', config.figFormat)), '-r300');
        end
        ...
        p1.EdgeColor = 'None';
        if ~ isempty(lidarConfig.markTRange)
            rectangle('Position', [tRangeMark(1), lidarConfig.hRange(2, 1), (tRangeMark(2) - tRangeMark(1)), (lidarConfig.hRange(2, 2) - lidarConfig.hRange(2, 1))], ...
                      'EdgeColor', 'k', ...
                      'LineWidth', 2, ...
                      'LineStyle', '--', ...
                      'FaceColor', [[193, 193, 193]/255, 0.5]);
        ...
        if exist(config.resultPath, 'dir')
            export_fig(gcf, fullfile(config.resultPath, sprintf('quicklook_%s_%s.%s', lidarType{iLidar}, '532s', config.figFormat)), '-r300');
        end
        ...
        p1.EdgeColor = 'None';
        if ~ isempty(lidarConfig.markTRange)
            rectangle('Position', [tRangeMark(1), lidarConfig.hRange(3, 1), (tRangeMark(2) - tRangeMark(1)), (lidarConfig.hRange(3, 2) - lidarConfig.hRange(3, 1))], ...
                      'EdgeColor', 'k', ...
                      'LineWidth', 2, ...
                      'LineStyle', '--', ...
                      'FaceColor', [[193, 193, 193]/255, 0.5]);
        ...
        if exist(config.resultPath, 'dir')
            export_fig(gcf, fullfile(config.resultPath, sprintf('quicklook_%s_%s.%s', lidarType{iLidar}, '532vdr', config.figFormat)), '-r300');
        end

        % diagnose signal merge
        if p.Results.flagDebug && p.Results.flagDebug
            displayREALSigMerge(lidarData.height, ...
                lidarData.mTime(isChosenMark), ...
                lidarData.sig532sh(:, isChosenMark), ...
                lidarData.sig532sl(:, isChosenMark), ...
                lidarData.mergeRange(1, :), ...
                'channelTag', '532S', ...
                'hRange', lidarConfig.hRange(1, :), ...
                'cRange', lidarConfig.sigRange(1, :), ...
                'mergeSlope', lidarData.mergeSlope(1), ...
                'mergeOffset', lidarData.mergeOffset(1), ...
                'figFolder', config.resultPath);
            displayREALSigMerge(lidarData.height, ...
                lidarData.mTime(isChosenMark), ...
                lidarData.sig532ph(:, isChosenMark), ...
                lidarData.sig532pl(:, isChosenMark), ...
                lidarData.mergeRange(2, :), ...
                'channelTag', '532P', ...
                'hRange', lidarConfig.hRange(2, :), ...
                'cRange', lidarConfig.sigRange(2, :), ...
                'mergeSlope', lidarData.mergeSlope(2), ...
                'mergeOffset', lidarData.mergeOffset(2), ...
                'figFolder', config.resultPath);
            displayREALSigMerge(lidarData.height, ...
                lidarData.mTime(isChosenMark), ...
                lidarData.sig607h(:, isChosenMark), ...
                lidarData.sig607l(:, isChosenMark), ...
        ...
        if ~ isempty(lidarConfig.markTRange)
            ...
            pSig = semilogx(rcs532pTmp, lidarData.height, ...
                'Color', [48, 80, 79]/255, ...
                'LineStyle', '-', ...
                'LineWidth', 2, ...
                'DisplayName', '532 P');
            hold on;
            pBg = semilogx(rcs532sTmp, lidarData.height, ...
                'Color', [177, 89, 41]/255, ...
                'LineStyle', '-', ...
                'LineWidth', 2, ...
                'DisplayName', '532 S');
            ...
            text(-0.1, -0.12, ...
                ...
                'FontWeight', 'Bold');
            text(0.3, 0.7, ...
                sprintf('From %s\nto %s\nProfiles: %d\n', datestr(tRangeMark(1), 'yyyy-mm-dd HH:MM'), datestr(tRangeMark(2), 'yyyy-mm-dd HH:MM'), sum(isChosenMark)), ...
                'Units', 'Normalized', ...
                'FontSize', 10, ...
                'HorizontalAlignment', 'left', ...
                'FontWeight', 'Bold');
            ...
            plot(vdr532Int, lidarData.height, 'Color', [231, 41, 139]/255, ...
                'LineStyle', '-', ...
                'LineWidth', 2);
            hold on;
            ...
            if exist(config.resultPath, 'dir')
                export_fig(gcf, fullfile(config.resultPath, sprintf('Profile_%s_%s.%s', lidarType{iLidar}, 'vdr532', config.figFormat)), '-r300');
            end
        end
    ...
    case {3}   % avors
        ...
        isChosen = (lidarData.mTime >= tRange(1)) & ...
                   (lidarData.mTime <= tRange(2));
        fprintf('Profiles for quicklook: %d\n', sum(isChosen));
        ...
        p1.EdgeColor = 'None';
        if ~ isempty(lidarConfig.markTRange)
            rectangle('Position', [tRangeMark(1), lidarConfig.hRange(1, 1), (tRangeMark(2) - tRangeMark(1)), (lidarConfig.hRange(1, 2) - lidarConfig.hRange(1, 1))], ...
                      'EdgeColor', 'k', ...
                      'LineWidth', 2, ...
                      'LineStyle', '--', ...
                      'FaceColor', [[193, 193, 193]/255, 0.5]);
    ...
    case {12, 13}   % WHU 1064 
        ...
        p1.EdgeColor = 'None';
        if ~ isempty(lidarConfig.markTRange)
            rectangle('Position', [tRangeMark(1), lidarConfig.hRange(1), (tRangeMark(2) - tRangeMark(1)), (lidarConfig.hRange(2) - lidarConfig.hRange(1))], ...
                      'EdgeColor', 'k', ...
                      'LineWidth', 2, ...
                      'LineStyle', '--', ...
                      'FaceColor', [[193, 193, 193]/255, 0.5]);
        ...
        if exist(config.resultPath, 'dir')
            export_fig(gcf, fullfile(config.resultPath, sprintf('quicklook_%s_%s.%s', lidarType{iLidar}, '1064e', config.figFormat)), '-r300');
        end
