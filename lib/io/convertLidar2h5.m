...
h5init(oFile);
hdf5writedata(oFile, '/time', datenum_2_unix_timestamp(inData.mTime), ...
    'dataAttr', struct('standard_name', 'time', ...
                       'Units', 'seconds since 1970-01-01 00:00:00 LT', ...
                       'calendar', 'julian'), ...
    'flagArray', true);
hdf5writedata(oFile, '/height', inData.height, ...
    'dataAttr', struct('short_name',' height'), ...
    'flagArray', true);

% write lidar data
for iCh = 1:size(inData.rawSignal, 1)
    rawSignal = reshape(inData.rawSignal(iCh, :, :), ...
        length(inData.height), length(inData.mTime));
    hdf5writedata(oFile, sprintf('/sig%s', chTag{iCh}), rawSignal(:, :), ...
        'dataAttr', struct('short_name', sprintf('signal_%s', chTag{iCh})));
