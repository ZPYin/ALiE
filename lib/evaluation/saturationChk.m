...
% saturationChk description
% USAGE:
%    [isPassSaturationChk] = saturationChk(lidarData, lidarConfig, reportFile)
% INPUTS:
...
for iCh = 1:length(lidarConfig.chTag)
    ...
    % quadrant time
    if ~ isempty(lidarConfig.saturationChkCfg.tRange{iCh})
    ...
    sigT1 = smooth(sigT1, swBins);
    sigT2Norm = smooth(sigT2 .* normRatio2, swBins);
    sigT3Norm = smooth(sigT3 .* normRatio3, swBins);
    sigT4Norm = smooth(sigT4 .* normRatio4, swBins);
    sigT5Norm = smooth(sigT5 .* normRatio5, swBins);
