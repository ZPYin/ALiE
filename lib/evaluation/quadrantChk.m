...
for iCh = 1:length(lidarConfig.chTag)
    ...
    sigNorth = smooth(nanmean(lidarData.(['rcs', lidarConfig.chTag{iCh}])(:, isNorth), 2), swBins);
    ...
    totalSigNorm = (sigEast1Norm + sigSouthNorm + sigWestNorm + sigNorthNorm) / 4;
    ...
    pEast1 = plot(sigEast1, lidarData.height, 'Color', [106, 142, 34]/255, 'LineStyle', '-', 'LineWidth', 2, 'DisplayName', 'East1'); hold on;
    pSouth = plot(sigSouth, lidarData.height, 'Color', [0, 191, 254]/255, 'LineStyle', '-', 'LineWidth', 2, 'DisplayName', 'South');
    pWest = plot(sigWest, lidarData.height, 'Color', [230, 216, 189]/255, 'LineStyle', '-', 'LineWidth', 2, 'DisplayName', 'West');
    pNorth = plot(sigNorth, lidarData.height, 'Color', [165, 118, 30]/255, 'LineStyle', '-', 'LineWidth', 2, 'DisplayName', 'North');
    pEast2 = plot(sigEast2, lidarData.height, 'Color', [254, 191, 111]/255, 'LineStyle', '--', 'LineWidth', 2, 'DisplayName', 'East2');
    ...
    if exist(p.Results.figFolder, 'dir')
        export_fig(gcf, fullfile(p.Results.figFolder, sprintf('telecover_test_rcs_near_%s_%s.%s', lidarType, lidarConfig.chTag{iCh}, p.Results.figFormat)), '-r300');
    end

    % normalized signal (near)
    figure('Position', [0, 10, 300, 400], 'Units', 'Pixels', 'Color', 'w', 'Visible', lidarConfig.figVisible);
    pTotal = plot(totalSigNorm, lidarData.height, 'Color', 'k', 'LineStyle', '-', 'LineWidth', 2, 'DisplayName', 'Total'); hold on;
