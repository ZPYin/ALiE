...
for iCh = 1:length(lidarConfig.chTag)
    ...
    tRange = [datenum(lidarConfig.detectRangeChkCfg.tRange(1:19), 'yyyy-mm-dd HH:MM:SS'), ...
              datenum(lidarConfig.detectRangeChkCfg.tRange(23:41), 'yyyy-mm-dd HH:MM:SS')];
    ...
    bg0 = bg;
    bg0(bg0 <= 0) = NaN;
    pSig = semilogx(sig0, lidarData.height, ...
        'Color', [48, 80, 79]/255, ...
        'LineStyle', '-', ...
        'LineWidth', 2, ...
        'DisplayName', 'signal');
    hold on;
    pBg = semilogx([bg0, bg0], [lidarData.height(1), lidarData.height(end)], ...
        'Color', [177, 89, 41]/255, ...
        'LineStyle', '-', ...
        'LineWidth', 2, ...
        'DisplayName', 'background');

    xlabel('Lidar signal (a.u.)');
    ylabel('Height (m)');
    text(1.15, 1.05, ...
        sprintf('Detection range test (%s, %s)', lidarType, lidarConfig.chTag{iCh}), ...
        'Units', 'Normalized', ...
        'FontSize', 12, ...
        'FontWeight', 'Bold', ...
    ...
    text(0.3, 0.7, ...
        sprintf('From %s\nto %s\nProfiles: %d\n', ...
            datestr(tRange(1), 'yyyy-mm-dd HH:MM'), ...
            datestr(tRange(2), 'yyyy-mm-dd HH:MM'), ...
            sum(isChosen)), ...
        'Units', 'Normalized', ...
        'FontSize', 10, ...
        'HorizontalAlignment', 'left', ...
    ...
    snrTmp = snr0;
    snrTmp(snrTmp <= 0) = NaN;
    semilogx(snrTmp, lidarData.height, ...
        'Color', [231, 41, 139]/255, ...
        'LineStyle', '-', ...
        'LineWidth', 2);
    hold on;

    if ~ isempty(lowSNRInd)
        p1 = plot([1e-10, 1e10], [lidarData.height(lowSNRInd), lidarData.height(lowSNRInd)], ...
            'Color', [177, 89, 41]/255, ...
            'LineStyle', '--', ...
            'LineWidth', 2, ...
            'DisplayName', sprintf('SNR = %4.1f', lidarConfig.detectRangeChkCfg.minSNR(iCh)));
        scatter(snrTmp(lowSNRInd), lidarData.height(lowSNRInd), 10, ...
            'Marker', 'o', ...
            'MarkerEdgeColor', [177, 89, 41]/255, ...
            'MarkerFaceColor', [177, 89, 41]/255);
