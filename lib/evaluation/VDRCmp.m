...
tRange = [datenum(config.externalChkCfg.VDRCmpCfg.tRange(1:19), 'yyyy-mm-dd HH:MM:SS'), ...
          datenum(config.externalChkCfg.VDRCmpCfg.tRange(23:41), 'yyyy-mm-dd HH:MM:SS')];
pSig = [];
sSig = [];
cmpVDR = [];
...
for iES = 1:nES
    isInES = (height >= config.externalChkCfg.VDRCmpCfg.hChkRange(iES, 1)) & ...
             (height <= config.externalChkCfg.VDRCmpCfg.hChkRange(iES, 2));
    ...
    for iLidar = 2:length(lidarType)
        fprintf(fid, 'Mean relative deviations of %s: %6.2f%% (max: %6.2f%%)\n', ...
            lidarType{iLidar}, ...
            meanVDRDev(iES, iLidar), ...
            config.externalChkCfg.VDRCmpCfg.maxDev(iES, 1));
        fprintf(fid, 'Mean relative deviations of %s: %6.2f%% (max: %6.2f%%)\n', ...
            lidarType{iLidar}, ...
            stdVDRDev(iES, iLidar), ...
            config.externalChkCfg.VDRCmpCfg.maxDev(iES, 1));
    end
end

%% data visualization
cmaps = myColormap('Chinese_colormap');
...
lineInstances = [];
lineInstances(1) = plot(cmpVDRSm(:, 1), height, ...
    'Color', 'k', ...
    'LineStyle', '-', ...
    'LineWidth', 2, ...
...
for iLidar = 2:length(lidarType)
    p1 = plot(cmpVDRSm(:, iLidar), height, ...
        'color', cmaps(floor((iLidar - 1) / (length(lidarType) - 1) * 44) + 7, :), ...
        'LineStyle', '-', ...
        'LineWidth', 2, ...
...
if exist(config.resultPath, 'dir')
    export_fig(gcf, fullfile(config.resultPath, sprintf('vdr_comparison.%s', config.figFormat)), '-r300');
end
...
for iES = 1:nES
    p3 = plot([-1, -1] * config.externalChkCfg.VDRCmpCfg.maxDev(iES, 1), ...
        config.externalChkCfg.VDRCmpCfg.hChkRange(iES, :), '--', ...
        'Color', [160, 160, 160]/255, ...
        'LineWidth', 2, ...
        'DisplayName', 'Mean Dev.');
    plot([1, 1] * config.externalChkCfg.VDRCmpCfg.maxDev(iES, 1), ...
        config.externalChkCfg.VDRCmpCfg.hChkRange(iES, :), '--', ...
        'Color', [160, 160, 160]/255, ...
        'LineWidth', 2);
...
for iPatch = 1:nES
    hShaded = patch(...
        [config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 1), ...
         config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 1), ...
         -config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 1), ...
         -config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 1)], ...
        [config.externalChkCfg.VDRCmpCfg.hChkRange(iPatch, 1), ...
         config.externalChkCfg.VDRCmpCfg.hChkRange(iPatch, 2), ...
         config.externalChkCfg.VDRCmpCfg.hChkRange(iPatch, 2), ...
         config.externalChkCfg.VDRCmpCfg.hChkRange(iPatch, 1)], [160, 160, 160]/255);
...
for iES = 1:nES
    p4 = plot([-1, -1] * config.externalChkCfg.VDRCmpCfg.maxDev(iES, 2), ...
        config.externalChkCfg.VDRCmpCfg.hChkRange(iES, :), '-.', ...
        'Color', [160, 160, 160]/255, ...
        'LineWidth', 2, ...
        'DisplayName', 'Standard Dev.');
    plot([1, 1] * config.externalChkCfg.VDRCmpCfg.maxDev(iES, 2), ...
        config.externalChkCfg.VDRCmpCfg.hChkRange(iES, :), '-.', ...
        'Color', [160, 160, 160]/255, ...
        'LineWidth', 2);
...
for iPatch = 1:nES
    hShaded = patch(...
        [config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 2), ...
         config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 2), ...
         -config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 2), ...
         -config.externalChkCfg.VDRCmpCfg.maxDev(iPatch, 2)], ...
        [config.externalChkCfg.VDRCmpCfg.hChkRange(iPatch, 1), ...
...
lineInstances1 = [];
for iLidar = 2:length(lidarType)
    p1 = scatter(meanVDRDev(:, iLidar), ...
        mean(config.externalChkCfg.VDRCmpCfg.hChkRange, 2), 25, ...
        'Marker', 's', ...
...
if exist(config.resultPath, 'dir')
    export_fig(gcf, fullfile(config.resultPath, sprintf('VDR_mean_deviation.%s', config.figFormat)), '-r300');
end
