...
% rangeCmp description
% USAGE:
%    [output] = rangeCmp(params)
% INPUTS:
%    params
% OUTPUTS:
%    output
...
tRange = [datenum(config.externalChkCfg.rangeCmpCfg.tRange(1:19), 'yyyy-mm-dd HH:MM:SS'), datenum(config.externalChkCfg.rangeCmpCfg.tRange(23:41), 'yyyy-mm-dd HH:MM:SS')];
cmpSig = [];
...
cmpSigNorm = cmpSig;
isNormRange = (height >= config.externalChkCfg.rangeCmpCfg.normRange(1)) & (height <= config.externalChkCfg.rangeCmpCfg.normRange(2));
if ~ any(isNormRange)
...
isHFit = (height >= config.externalChkCfg.rangeCmpCfg.hRange(1)) & (height <= config.externalChkCfg.rangeCmpCfg.hRange(2));
if ~ any(isHFit)
...
% fit range
plot(config.externalChkCfg.rangeCmpCfg.sigRange, [1, 1] * config.externalChkCfg.rangeCmpCfg.fitRange(1), '--', 'Color', [152, 78, 163]/255, 'LineWidth', 2);
plot(config.externalChkCfg.rangeCmpCfg.sigRange, [1, 1] * config.externalChkCfg.rangeCmpCfg.fitRange(2), '--', 'Color', [152, 78, 163]/255, 'LineWidth', 2);
